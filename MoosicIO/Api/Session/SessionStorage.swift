...
protocol SessionsStorage {
    ...
    func remove(sessions: [EncodedSession]) throws
}

final class SessionsStorageImpl: SessionsStorage {
    private let fileManager: MoosicFileManager
    private let gateQueue = DispatchQueue(label: "MoosicIO.sessionStorageQueue")
    private let bundleName: String
    private let configName: String
    
    init(fileManager: MoosicFileManager, bundleName: String, configName: String) {
    ...
    func restore() throws -> [EncodedSession] {
        return try gateQueue.sync {
            ...
            guard fileManager.fileExists(atPath: fileUrl.path) else {
                print("MoosicIO: Session not restored because file not exists yet")
                return []
            }
    ...
    func configurationUrl() throws -> URL {
        ...
        return try fileManager.url(for: .applicationSupportDirectory, in: .userDomainMask, appropriateFor: nil, create: true).appendingPathComponent(bundleName).appendingPathComponent("\(configName).plist")
    }
